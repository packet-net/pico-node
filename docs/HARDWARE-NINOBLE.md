...
- **TNC firmware.** Updates the NinoTNC over the J5 serial link, so the USB chip is not needed. Either press **Get it from GitHub** (the browser fetches `N9600A-v3-44.hex` or `-v4-44.hex` from [flashtnc](https://github.com/ninocarrillo/flashtnc) to match the TNC's chip) or choose the file and press **Upload to the node**. The node checks every line (record checksums, the end-of-file record, and the chip fingerprint that tells a 3.x file from a 4.x one) while storing it in its APPDATA flash region (about 374 KB). Then **Update the TNC** runs the flashtnc procedure from the node: quiet the line, enter the TNC's bootloader, check the bootloader is for the same chip as the file, and send the file a line at a time. It takes a few minutes with the TNC off the air. A wrong-chip file is refused before anything is written. If an update is interrupted part way, the TNC waits in its bootloader (LEDs dark); pressing **Update the TNC** again finishes it. Afterwards the node sends its saved mode and KISS parameters to the updated TNC.
