...
- **Monitor.** Frames heard (RX), frames sent (TX) and TNC events such as mode changes and status reports, updated every second. **Ask the TNC for its status** requests a fresh report and shows the answer under the button (firmware version, DIP position, running mode), or says there was none.
